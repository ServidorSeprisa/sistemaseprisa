...
class FormatoRecepcionMateriaPrimaCreateView(generic.CreateView):
    model = FormatoRecepcionMateriaPrima
    fields = ('fechaentrada','materiaprima', 'loteseprisa', 'pesobruto', 'pesoneto', 'nocontenedores','cantidadneto', 'claveproveedor', 'noanalisis', 'sku', 'noloteproveedor', 'fechacaducidad', 'recibe')
...
def export_kardex_pdf(request):
    etiqueta_id = request.GET.get('id')
    etiqueta = FormatoRecepcionMateriaPrima.objects.filter(id=etiqueta_id).first()
...
class FormatoBitacoraProductoTerminadoListViewView(ListView):
    model = FormatoBitacoraProductoTerminado
    template_name = 'contacts/formato_bitacora_producto_terminado_view.html'
    context_object_name = "object_list"
    paginate_by = 15  
...
def export_orden_produccion_create_listview_pdf(request, pk):
    ...
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="OrdenProduccion_{orden.noordenproduccion}.pdf"'
...
def export_control_aseguramiento_pdf(request):
    ...
    if search_query:
        try:
            ...
            muestras1 = Muestra.objects.filter(
                orden_produccion=orden_produccion,
                columna__gte=1,
                columna__lte=13
            ).order_by('fila', 'columna')

            muestras2 = Muestra.objects.filter(
                orden_produccion=orden_produccion,
                columna__gte=14,
                columna__lte=26
            ).order_by('fila', 'columna')
            ...
            for fila in range(1, 11):
                if fila not in filas1:
                    filas1[fila] = [None] * 13
            tabla1_muestras = [{'fila': fila, 'valores': filas1[fila]} for fila in range(1, 11)]
            ...
            for fila in range(1, 11):
                if fila not in filas2:
                    filas2[fila] = [None] * 13
            tabla2_muestras = [{'fila': fila, 'valores': filas2[fila]} for fila in range(1, 11)]
...
def export_control_aseguramiento2_pdf(request, orden_produccion_id):
    orden_produccion = get_object_or_404(OrdenProduccion, id=orden_produccion_id)
    ...
    num_muestras = 26
    sumas = [0] * num_muestras
    counts = [0] * num_muestras

    # Para tabla 1 (columnas 1-13)
    for muestra in muestras1:
        index = muestra.columna - 1  # índice 0-12
        valor = muestra.valor or 0
        sumas[index] += valor
        if valor != 0:
            counts[index] += 1

    # Para tabla 2 (columnas 14-26)
    for muestra in muestras2:
        index = muestra.columna - 1  # índice 13-25
        valor = muestra.valor or 0
        sumas[index] += valor
        if valor != 0:
            counts[index] += 1
    ...
    for i in range(num_muestras):
        promedio = (sumas[i] / counts[i]) if counts[i] > 0 else 0
        promedios_totales.append(round(promedio, 2))

    # Generar gráfica
    grafica_base64 = generar_grafica_base64(promedios_totales)
...
import matplotlib
matplotlib.use('Agg')  # 🔥 Esto evita que use Tkinter
import matplotlib.pyplot as plt
import io
import base64

def generar_grafica_base64(promedios_totales):
    x = list(range(1, len(promedios_totales) + 1))  # Muestras 1, 2, 3 ... 26

    plt.figure(figsize=(10, 5))
    plt.plot(x, promedios_totales, marker='o', label='Promedios Totales', color='purple')
    plt.title('Gráfica de Control de Calidad')
    plt.xlabel('Muestra')
    plt.ylabel('Promedio')
    plt.xticks(x, [f'Muestra {i}' for i in x], rotation=45)
    plt.legend()
    plt.tight_layout()
